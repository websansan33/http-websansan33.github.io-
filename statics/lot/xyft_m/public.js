...
{case((H>=5&&H<13)||(H==4&&N>4)||(H==13&&N<9)):Qi=1;Kai=new Date(Y,M,D,13,09,0);break;case(H>13||(H==13&&N>=9)):var firstDT=new Date(Y,M,D,13,9,0).getTime();Qi=Math.ceil((T-firstDT)/perQiSec/1000)+1;Kai=new Date(Y,M,D,13,9+(Qi-1)*5,0);break;default:reQi=Y.toString()+addZero(M+1)+addZero(D-1)+"-";var firstDT=new Date(Y,M,D,0,0-1,0).getTime();Qi=131+Math.ceil((T-firstDT)/perQiSec/1000);Kai=new Date(Y,M,D,0,((Qi-131)*5)-1,0);break;}
...
{$.ajax({url:codeurl+'?ts'+$.now(),type:"post",dataType:"xml",timeout:6000,contentType:"application/x-www-form-urlencoded; charset=utf-8",async:true,error:function(xml,msg){},success:function(xml){$("#codeTab").find(".currenAward").html('');if($("#codeTab")||$("#botCodeTab"))
...
$("#openq").empty();$("#openq").html(strOpenCode+'<b>'+$(this).attr("z")+'<b>');strOpenCode=null;ayyhit_code=null;var qi_cmp=nowQi-yyhit_qistrs;if(qi_cmp==1&&intervalId>0)
