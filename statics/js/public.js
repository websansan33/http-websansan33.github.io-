...
{
  //刷新验证码
  window.onunload = function(){};//fix firefox opera
...
{//多少个号码 2位一个号码 如双色球
  var sArr = s.split(",");
  for(var i = 0;i < sArr.length;i++) {if(sArr[i].length != 2) return 0;}
  return sArr.length;
}
/*
...
function showLoginPanle() { //检测登录状态并显示登录小窗口
...
function dealLoginPage(flag) { //登录或退出后刷新页面
